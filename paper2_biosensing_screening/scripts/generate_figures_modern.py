...
def fig2_workflow():
    ...
    _flow_arrow(ax, 3.3, 8.45, 3.3, 7.15, blue)
    _flow_arrow(ax, 6.7, 8.45, 6.7, 7.15, orange)

    _card(ax, 3.05, 6.35, 4.3, 1.45,
          ["Molecular recognition",
           "blind docking (smina, seeded)",
           "4 disease targets + calibration"], blue, bluef, fs_pt=7.6)
    _card(ax, 6.95, 6.35, 4.3, 1.45,
          ["Optical sensitivity",
           "TD-DFT solvatochromic shift",
           "CPCM toluene/water, 2 functionals"], orange, orangef, fs_pt=7.6)

    _flow_arrow(ax, 3.05, 5.6, 3.05, 4.35, blue)
    _flow_arrow(ax, 6.95, 5.6, 6.95, 4.35, orange)

    _card(ax, 3.05, 3.6, 4.3, 1.35,
          ["Result", "4 stable donors bind", "within inhibitor band"], blue, bluef, fs_pt=7.6)
    _card(ax, 6.95, 3.6, 4.3, 1.35,
          ["Result", "large shift only in the", "reactive set (dark states)"], orange, orangef, fs_pt=7.6)

    _flow_arrow(ax, 3.05, 2.85, 4.55, 1.75, "#4a5568")
    _flow_arrow(ax, 6.95, 2.85, 5.45, 1.75, "#4a5568")

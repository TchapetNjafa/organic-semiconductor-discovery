#!/usr/bin/env python3
"""
Paper 2 data figures, JMC-C reframe (10-molecule candidate set), 2026-08-04.
Extends generate_figures_modern-V2.py (the 7-molecule JCIM version) with the 3
gap-filling candidates (20549, 5081, 23424) added to close the heavy-atom-count
confound (ledger L-2026-08-03-02/-03/-04). Docking now reads the apo-vs-apo
dataset (docking_results_apo.csv, fixes JCIM concern #2) instead of the old
holo-based docking_results_clean.csv. Solvatochromic data merges the original
7-candidate CSVs with the 3 new gap-candidate CSVs. NO value is hardcoded
except the ten already-ledgered dipole magnitudes (L-2026-08-03-02 for the 3
new ones; companion Paper 1 dataset for the original 7, unchanged).
...
(blue family = 5 stable donors, orange/red/purple family = 5 reactive structures).
"""
import csv
...
# ----------------------------------------------------------------------------
STABLE = ["1712", "9168", "17574", "18506", "20549"]
REACTIVE = ["4550", "17851", "20778", "5081", "23424"]
...
DIPOLE = {  # Debye, dataset_pubchemqc_opv_17458.csv col dipole_moment
    ...
    # 3 gap-filling candidates, ledger L-2026-08-03-02
    "20549": 2.96, "5081": 2.42, "23424": 8.17,
}
...
SMILES = {
    ...
    # 3 gap-filling candidates, SMILES from genuine_osc_ranked.csv, verified by
    # RDKit heavy-atom count against the ledger (docking_rerun/dock_gap_candidates_apo.py)
...
# Colorblind-safe two-hue scheme: blues = stable, oranges/red/purple = reactive.
CLR = {
    "1712":  "#0072B2",  # blue
    "9168":  "#009E73",  # teal-green
    "17574": "#56B4E9",  # light blue
    "18506": "#1b4965",  # deep navy
    "20549": "#7A9CC6",  # steel blue (new stable candidate)
    "4550":  "#D55E00",  # vermillion
    "17851": "#E69F00",  # amber
    "20778": "#B30000",  # deep red
    "5081":  "#CC79A7",  # reddish purple (new reactive candidate, Okabe-Ito safe)
    "23424": "#8B4513",  # saddle brown (new reactive candidate)
}
STABLE_INK = "#0072B2"
REACTIVE_INK = "#D55E00"
GRID = "#e3e7ee"
...
def fig2_workflow():
    ...
    _card(ax, 5.0, 9.1, 8.6, 1.25,
          ["Companion screen: 17,458 molecules",
           "chemical-validity + stability gate  →  7 candidates + 3 gap-fillers = 10"],
    ...
    _card(ax, LX, 6.35, BRANCH_W, 1.45,
          ["Molecular recognition",
           "apo-receptor blind docking (smina, seeded)",
           "4 disease targets + calibration"], blue, bluef, fs_pt=7.6)
    _card(ax, RX, 6.35, BRANCH_W, 1.45,
          ["Optical sensitivity",
           "TD-DFT solvatochromic shift + diffuse-basis check",
           "CPCM toluene/water, 2 functionals, 6-31G*/6-31+G*"], orange, orangef, fs_pt=7.6)

    _flow_arrow(ax, LX, 5.6, LX, 4.35, blue)
    _flow_arrow(ax, RX, 5.6, RX, 4.35, orange)

    _card(ax, LX, 3.6, BRANCH_W, 1.35,
          ["Result", "5 stable donors bind", "within inhibitor band"], blue, bluef, fs_pt=7.6)
    ...
    _card(ax, 5.0, 1.05, 7.0, 1.25,
          ["Class separation now overlaps in size and affinity, not just relabeled",
           "two readouts still select different subsets, reported side by side"], teal, tealf, fs_pt=8.0)

    save(fig, "fig2_workflow")


def load_docking():
    path = BASE / "docking_rerun" / "docking_results_apo.csv"
...
def load_s1(functional):
    ...
    Merges the original-7 CSV with the 3 gap-candidate CSV (ledger
    L-2026-08-03-04); both share an identical schema and level of theory
    (B3LYP/6-31G* and CAM-B3LYP/6-31G*, TDA, CPCM)."""
    paths = [
        BASE / "solvatochromic_tddft" / f"solvatochromic_results_{functional}.csv",
        BASE / "solvatochromic_tddft" / f"solvatochromic_results_gap_candidates_{functional}.csv",
    ]
    s1 = {}
    for path in paths:
        with open(path) as fh:
...
def fig1_graphical_abstract():
    ...
    ax.text(0.5, 0.965, "Ten organic-semiconductor candidates, two computational readouts",
    ...
    ax.text(0.255, 0.875, "Predicted protein binding (apo)", ha="center", va="center",
            fontsize=7.6, fontweight="bold", color=blue, zorder=6)
    for mid, xx in zip(STABLE, [0.048, 0.128, 0.208, 0.288, 0.368]):
        _mol_chip(ax, mid, xx, 0.615, 0.078, 0.215, blue)
    ax.add_patch(FancyBboxPatch((0.06, 0.45), 0.39, 0.12,
                 boxstyle="round,pad=0,rounding_size=0.02",
                 facecolor="#c9d6e5", edgecolor="none", alpha=0.75, zorder=2))
    ax.text(0.445, 0.575, "native-inhibitor range", ha="right", va="bottom",
            fontsize=6.3, style="italic", color="#4a5568", zorder=6)
    for i in range(5):
        ax.scatter(0.09 + i * 0.068, 0.51, s=72, marker="o", color=blue,
                   edgecolor="white", linewidth=1.0, transform=ax.transAxes, zorder=5)
    ax.text(0.255, 0.31, "5 stable donors bind within\nthe known-inhibitor range",
    ...
    ax.text(0.745, 0.875, "Optical environment-sensitivity", ha="center", va="center",
            fontsize=7.6, fontweight="bold", color=orange, zorder=6)
    for mid, xx in zip(REACTIVE, [0.538, 0.618, 0.698, 0.778, 0.858]):
        _mol_chip(ax, mid, xx, 0.615, 0.088, 0.215, orange)
    ...
    ax.text(0.745, 0.31, "shift is large in most reactive\nstructures, but not all (class ≠ signal)",
            ha="center", va="center", fontsize=6.6, color=INKTXT, zorder=6)
...
def fig7_structures():
    # 2 rows x 5 cols; row 0 = 5 stable donors, row 1 = 5 reactive structures
    fig = plt.figure(figsize=(DOUBLE, DOUBLE * 0.48))
    gs = fig.add_gridspec(2, 5, hspace=0.30, wspace=0.08,
